...
def gen_rss(data, title, url):
    ...
    rss = PyRSS2Gen.RSS2(
        title=title,
        link=url,
        description="New RSS for " +
        title +
        " (Generate by Pendekar Langit)",
        lastBuildDate=datetime.now(),
        items=[
            PyRSS2Gen.RSSItem(
                title=d[0],
                link=d[1],
                description=list_to_str(
                    d[2]),
                pubDate=d[3]) for d in data],
    )
    rss.write_xml(
        open(
            url.replace(
...
def auto_ping(title, url, url_rss):
    ...
    for x in range(1, 10):
        requests.get(
            ...
            "&rssurl=" +
            url_rss +
            "&chk_weblogscom=on&chk_blogs=on&chk_feedburner=on&chk_newsgator=on&chk_myyahoo=on&chk_pubsubcom=on&chk_blogdigger=on&chk_weblogalot=on&chk_newsisfree=on&chk_topicexchange=on&chk_google=on&chk_tailrank=on&chk_skygrid=on&chk_collecta=on&chk_superfeedr=on")
        time.sleep(900)
    ...
    while True:
        ...
        if len(urls) > 0:
            for url in urls:
                ...
                data = parser.data
                data_rss.set_items(
                    data['items']['title'], url, data['items']['desc'])
